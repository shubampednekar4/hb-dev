...
class TrainingVideosController extends AppController
{
    ...
    public function index()
    {
        ...
        $videos = $this->TrainingVideos->Videos->find();
        $this->set(compact('videos', 'resetNeeded', 'search'));
    }
    ...
    {
        $video = $this->TrainingVideos->Videos->get($id);
        $this->Authorization->skipAuthorization();
        $identity = $this->Authentication->getIdentity();
        $identifier = $identity->getIdentifier();
        $user = $this->TrainingVideos->Operators->Users->get($identifier);
        $this->set(compact('video', 'user'));
    }
    ...
    public function add()
    {
        ...
        $identity = $this->Authentication->getIdentity();
        $identifier = $identity->getIdentifier();
        $video = $this->TrainingVideos->Videos->newEmptyEntity();
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($data['thumbnail_location'])) {
                $imageFile = $data['thumbnail_location'];
                $imagePath = WWW_ROOT . 'videos' . DS . 'uploads' . DS;
                $imageFilename = time() . '_' . $imageFile->getClientFilename();
                $imageFile->moveTo($imagePath . $imageFilename);
                $video->thumbnail_location = $imageFilename;
            }
            else {
                $video->thumbnail_location = ''; // Set a default value
            }
            
            $video->created_at = date('Y-m-d H:i:s');
            if ($this->TrainingVideos->Videos->save($video)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            $video = $this->TrainingVideos->Videos->patchEntity($video, $this->request->getData());
            if ($this->TrainingVideos->Videos->save($video)) {
    ...
    public function delete($id = null)
    {
        ...
        $identity = $this->Authentication->getIdentity();
        $identifier = $identity->getIdentifier();
        if ($this->TrainingVideos->Videos->delete($video)) {
